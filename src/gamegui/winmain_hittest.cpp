...
static GG_Player *g_player = null;
...
//  UpdateWindow( hWnd );
  ...
  if( gg->createPlayer( movie, &g_player ) != GG_OK )
  {
  ...
  g_player->setPlayMode(GG_PLAYMODE_LOOP);        //  Tell our movie to loop
  ...
  switch ( message )
  {
    ...
    case WM_RBUTTONDOWN:
      if( g_player != null )
      {
        ...
        x = LOWORD(lParam);
        y = HIWORD(lParam);
        if( g_player->detectHiti( x, y, GGID_STOPBUTTON ) )
          PostQuitMessage(0);                 //  Exit if they hit the stop button
      }
      return 0;
